...
export default function Home() {
  ...
  // --- ⭐️ إضافة الشعار وعنوان المتصفح ⭐️ ---
  useEffect(() => {
    // 1. تعيين عنوان الصفحة في النافذة
    document.title = "porn4games";

    // 2. البحث عن رابط الأيقونة الحالي
    let link = document.querySelector("link[rel~='icon']");
    
    // 3. إذا لم يوجد، قم بإنشاء واحد
    if (!link) {
      link = document.createElement('link');
      link.rel = 'icon';
      // إضافة الرابط إلى <head>
      document.getElementsByTagName('head')[0].appendChild(link);
    }
    
    // 4. تعيين مسار الأيقونة (من مجلد public)
    link.href = '/logo.png';
  }, []); // <-- المصفوفة الفارغة تضمن تشغيله مرة واحدة عند تحميل المكون
